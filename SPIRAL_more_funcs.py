...
def visualize_multiple_structures(data_name,cell_selection,ordered_pairs,impute_method,structs_to_vis,
    ...
    if no_iterations:
        if ordered_pairs:
            sigfile_filt = ('../results_no_iterations/sigtables/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+'/ordered_pairs/'+
                            cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+'_significance_table_'+filt_mode+
                            '.xlsx')
        else:
            sigfile_filt = ('../results_no_iterations/sigtables/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+'/unordered_pairs/'+
                            cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+'_significance_table_'+filt_mode+
                            '.xlsx')
    else:
        if ordered_pairs:
            sigfile_filt = ('../results/sigtables/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+'/ordered_pairs/'+
                            cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+'_significance_table_'+filt_mode+
                            '.xlsx')
        else:
            sigfile_filt = ('../results/sigtables/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+'/unordered_pairs/'+
                            cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+'_significance_table_'+filt_mode+
                            '.xlsx')
    ...
    if len(sigtable)>0:
        ...
        # Load genes_table
        with open('./objects/genes_tables/'+data_name+'_'+str(num_genes)+'X'+str(num_cells)+'_'+filter_word+'.p', 'rb') as fp:
        ...
        if read_scRNA_data_from_file:
            ...
            imputed_file = data_dict[data_name][:ind+1]+impute_method+data_dict[data_name][ind:-4]+'_'+filter_word+'_imputed.txt'
        ...
        if data_name=='RinaBulkRNAData':
            ...
            print(cell_definitions)
            cell_labels = np.zeros((len(list(scRNA_data)))).astype(int)
            for p,patient in enumerate(['hc', 'cl', 'j', 'n984', 'n4', 'n995', 'n962', 'neg1', 'neg2', 'pos_l', 'pos_h']):
            ...
            if (impute_method=='Iacono_clustering_dimcutoff_50') or (impute_method=='Iacono_clustering_dimcutoff_50_median'):
                clusters = pd.read_csv('../representing_clusters/Iacono_clusters_dimcutoff_50/'+data_name+'_'+filter_word+'_clusters.txt', sep=' ', index_col=0)
            else:
                clusters = pd.read_csv('../representing_clusters/Iacono_clusters/'+data_name+'_'+filter_word+'_clusters.txt', sep=' ', index_col=0)
            ...
            cell_definitions = [{0: 'CD8 T cell', 1: 'monocyte', 2: 'dendritic cell', 3: 'B cell', 4: 'CD4 T cell', 
                                 5: 'unknown', 6: 'dendritic cell',7: 'monocyte', 8: 'NK cell'}[x] for x in cell_labels]        
            ...
            cell_definitions = [{0: 'NK cell', 1: 'monocyte',  2: 'unknown', 3: 'CD4 T cell', 4: 'monocyte',  5: 'B cell', 
                                 6: 'NK cell',7: 'monocyte',8: 'NK cell',9: 'T cell', 10: 'dendritic cell',11: 'B cell', 
    ...
    for i in structs_to_vis:
        ...
        sets_in_struct = [tuple(list(map(int, s.strip(')').strip('(').split(', ')))) for s in sets_in_struct.strip(']').strip('[').split('), (')]
        ...
        if not general_graphs:
            ...
            nx.draw(G, pos, node_color=np.array(cell_labels)[G.nodes], node_size=800, cmap=plt.cm.Blues, with_labels=True, 
                    arrows=True, labels={
        ...
        if ordered_pairs:
            picfolder = ('../results/structure_visualizations/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+
...
def visualize_multiple_structures_in_one_figure(data_name,cell_selection,ordered_pairs,impute_method,
    ...
    ind = [m.start() for m in re.finditer('/', data_dict[data_name])][-1]
    umapcoorfile = data_dict[data_name][:ind+1]+impute_method+data_dict[data_name][ind:-4]+'_'+filter_word+'_imputed_umap.csv'
    ...
    # cell labels
    ind = [m.start() for m in re.finditer('/', data_dict[data_name])][-1]
    celllabelsfile = data_dict[data_name][:ind+1]+impute_method+data_dict[data_name][ind:-4]+'_'+filter_word+'_imputed_cell_labels.txt'
    celldeffsfile = data_dict[data_name][:ind+1]+impute_method+data_dict[data_name][ind:-4]+'_'+filter_word+'_imputed_cell_deffs.txt'
    ...
    # picture file name
    picname = (data_name+'_'+filter_word+'_'+impute_method+'_'+str(num_stds_thresh_lst)+'_'+str(mu_lst)+'_structs_'+
...
def save_ranked_gene_lists(data_name,cell_selection,ordered_pairs,impute_method,
    ...
    if no_iterations:
        if ordered_pairs:
            structs_filename=('./objects_no_iterations/struct_lists/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+
                      '/ordered_pairs/'+cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+
        ...
        if ordered_pairs:
            structs_filename=('./objects/struct_lists/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+'/ordered_pairs/'+
                          cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+'_relevant_structs.txt')
        else:
            structs_filename=('./objects/struct_lists/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+'/unordered_pairs/'+
                          cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+'_relevant_structs.txt')
    ...
    if no_iterations:
        if ordered_pairs:
            ranked_gene_list_folder = ('../results_no_iterations/ranked_gene_lists/num_stds_thresh_'+str(num_stds_thresh)+
        ...
        if ordered_pairs:
            ranked_gene_list_folder = ('../results/ranked_gene_lists/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+
    ...
    for i in structs_lst:
        ...
        #print(sets_in_struct)
        sets = [tuple(list(map(int, s.strip(')').strip('(').split(', ')))) for s in sets.strip(']').strip('[').split('), (')]
        print(len(sets), 'sets')
        ...
        genes_probs_logs_table1.to_csv(ranked_gene_list_folder+'/struct_'+str(i)+'_ranked_genes.txt', sep='\t', index=False)
...
def compute_structure_average_std(data_name,cell_selection,ordered_pairs,impute_method,
    ...
    if ordered_pairs:
        sigfile = ('../results/sigtables/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+'/ordered_pairs/'+
                   cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+'_significance_table.xlsx')
    else:
        sigfile = ('../results/sigtables/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+'/unordered_pairs/'+
                   cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word+'_significance_table.xlsx')
    ...
    for i in list(sigtable.index):
        ...
        sets_in_struct = sigtable.loc[i,'sets']
        sets = [tuple(list(map(int, s.strip(')').strip('(').split(', ')))) for s in sets_in_struct.strip(']').strip('[').split('), (')]
        print('sets:', sets)
...
def find_opposite_structures(data_name,cell_selection,ordered_pairs,impute_method,read_sigtable_from_file=False,
    ...
    opporiste_structs_table = pd.DataFrame(np.zeros((100,24)), 
                                  columns=[['num_genes','num_cells','num_sets','struct1_ind','struct2_ind']+
    ...
    for k,i in enumerate(struct_inds):
        ...
        for t,j in enumerate(struct_inds[k+1:]):
            ...
            sets_i = [tuple(list(map(int, s.strip(')').strip('(').split(', ')))) for s in sets_i.strip(']').strip('[').split('), (')]
            print('sets_i:', sets_i)
            
            sets_j = sigtable.loc[j,'sets']
            sets_j = [tuple(list(map(int, s.strip(')').strip('(').split(', ')))) for s in sets_j.strip(']').strip('[').split('), (')]
            print('sets_j:', sets_j)
            ...
            if len(set(sets_i).intersection(set(sets_j_opposite)))/len(set(sets_i).union(set(sets_j_opposite)))>0.9:
                opporiste_structs_table.loc[opp_ind,['num_genes','num_cells','num_sets']] = sigtable.loc[i,['num_genes','num_cells','num_sets']]
                opporiste_structs_table.loc[opp_ind,'struct1_ind'] = i
                opporiste_structs_table.loc[opp_ind,'struct2_ind'] = j
                opporiste_structs_table.loc[opp_ind,['struct1_'+s for s in ['num_rows_in_struct','num_cols_in_struct',
                                        'num_cells_in_struct',
                                       '#1a_log10_multiplication_of_Chebyshev_bound_of_all_genes',
                                       '#1b_log10_factorial_of_num_rows_in_struct',
                                       '#2_log10_init_prob',
                                       ...
                                       'log10_pmax_of_genes','log10_pavg_of_genes',
                                       'structure_average_std',
                                       'genes','sets','graphs']]] = sigtable.loc[i,['num_rows_in_struct','num_cols_in_struct', 
                                       'num_cells_in_struct',
                                       '#1a_log10_multiplication_of_Chebyshev_bound_of_all_genes',
                                       '#1b_log10_factorial_of_num_rows_in_struct',
                                       '#2_log10_init_prob',
                                       ...
                                       'structure_average_std',
                                       'genes','sets','graphs']]
                opporiste_structs_table.loc[opp_ind,['struct2_'+s for s in ['num_rows_in_struct','num_cols_in_struct', 
                                       'num_cells_in_struct',
                                       '#1a_log10_multiplication_of_Chebyshev_bound_of_all_genes',
                                       '#1b_log10_factorial_of_num_rows_in_struct',
                                       '#2_log10_init_prob',
                                       ...
                                       'log10_pmax_of_genes','log10_pavg_of_genes',
                                       'structure_average_std',
                                       'genes','sets','graphs']]] = sigtable.loc[j,['num_rows_in_struct','num_cols_in_struct', 
                                       'num_cells_in_struct',
                                       '#1a_log10_multiplication_of_Chebyshev_bound_of_all_genes',
                                       '#1b_log10_factorial_of_num_rows_in_struct',
                                       '#2_log10_init_prob',
...
def visualize_multiple_structures_with_arrows(data_name,cell_selection,ordered_pairs,impute_method,structs_lst,
    ...
    if mode=='umap':            
        ...
        ind = [m.start() for m in re.finditer('/', data_dict[data_name])][-1]
        pcacoorfile = data_dict[data_name][:ind+1]+impute_method+data_dict[data_name][ind:-4]+'_'+filter_word+'_imputed_pca.csv'
    ...
    if no_iterations:
        if ordered_pairs:
            picfolder = ('../results_no_iterations/structure_visualizations/num_stds_thresh_'+str(num_stds_thresh)+'/mu_'+str(mu)+
                         '/ordered_pairs/'+cell_selection+'/'+impute_method+'/'+data_name+'_'+filter_word)
    ...
    temp = np.sqrt(np.power(radius[1],2)*(np.power(radius[0],2)-np.power(tempx-mid[0],2))/(np.power(radius[0],2)))
    ...
    # picture file name
    if red_only:
        picname = data_name+'_'+filter_word+'_'+impute_method+'_'+str(num_stds_thresh)+'_'+str(mu)+str(structs_lst)+'_'+mode+'_arrows_red_only.jpg'
    else:
        picname = data_name+'_'+filter_word+'_'+impute_method+'_'+str(num_stds_thresh)+'_'+str(mu)+str(structs_lst)+'_'+mode+'_arrows.jpg'
