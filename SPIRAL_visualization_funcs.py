...
##################################################################################################################
def compute_orig_deffs(data):
    if not all('_' in s for s in list(data)):
        return ['0']*data.shape[1]
    orig_deffs = [s[:s.find('_')] for s in list(data)]
    return orig_deffs

##################################################################################################################
def save_orig_deffs_to_file(data, data_norm_filt_loc, origdeffsfile):
    # load count matrix
    if data is None and data_norm_filt_loc is None:
        print('No data path and no data argument.')
    ...
    orig_deffs = compute_orig_deffs(data)
            ...
            if inds:
                ...
                if level == 'not in structure':
                    h = ax.scatter(coor[inds, 0], coor[inds, 1], color=color,
                                   label=(len(cell_definitions_sorted_set) != 1) * (deff + ' - ') + level_desc_dict[level],
                                   s=size, marker=shape)
                    hs.append(h)
...
def visualize_structures(sigfile_vis, genetable_file, repcellsumapcoorfile, repcellspcacoorfile,
                         ...
                         struct_thr=1.2, min_nstructs=3, max_nstructs=15, impute_method='agg_wald',
    ...
    # decide on structures to be visualized
    ...
    if len(structs_lst) > max_nstructs:
        ...
        structs_lst = list(sigtable.sort_values(by='structure_average_std', ascending=True).index[:min(min_nstructs, len(sigtable))])
    ...
    for i in structs_lst:
        ...
        title = ('Algorithm parameters: ' + r'$\alpha$' + '=' + str(sigtable.loc[i, 'num_stds_thresh']) + ', '
                 ...
                 ', ' + r'$\widetilde{\sigma}$' + '=' + str(np.around(sigtable.loc[i, 'structure_average_std'], 2)) +
                 '\nThe structure consists of ' + str(int(sigtable.loc[i, 'num_genes_in_struct'])) + '/' + str(
                    sigtable.loc[i, 'num_genes']) +
                 ' genes and ' + str(int(sigtable.loc[i, 'num_' + samp_name + 's_in_struct'])) + '/' +
                ...
                if len(set(deffs)) == 1:
                    ...
                    for deff, shape in zip(cell_definitions_sorted_set, shapes[:len(set(deffs))]):
                        ...
                        nx.draw_networkx_nodes(G, pos, nodelist=nodelist,
                                               node_size=700+np.array([50*(num_arrows_per_cell[node]-1) for node in nodelist]),
        ...
        if save_UMAPs_repcells or save_PCAs_repcells or save_UMAPs_orig or save_PCAs_orig or save_spatial:
            ...
            if (save_UMAPs_orig or save_PCAs_orig or save_spatial) and impute_method != 'no_imputation':
                ...
                levels_orig = [node_level_name_dict[int(cells_to_repcells.loc[i, 'x'])] for i in cells_to_repcells.index]
