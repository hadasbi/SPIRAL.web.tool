...
def find_submat_around_g(g,mu,abs_genes_in_sets):
    ...
    while not converged:
        ...
        new_cols_g = list(np.where(np.ravel(np.sum(abs_genes_in_sets[new_rows_g,:],axis=0))>=mu*len(new_rows_g))[0])
...
def find_submat_around_random_cellpair_set(mu,abs_genes_in_sets):
    ...
    while not converged:
        ...
        new_rows_g = list(np.where(np.ravel(np.sum(abs_genes_in_sets[:,new_cols_g],axis=1))>=mu*len(new_cols_g))[0])
        if new_rows_g!=rows_g or new_cols_g!=cols_g:
            cols_g = new_cols_g
            rows_g = new_rows_g
        else:
...
def compute_sets(genes_table,scRNA_data,gene_std,cell_pair,num_cells,ordered_pairs):
    ...
    if not ordered_pairs:
        gene_inds1 = np.array(list(genes_table.loc[scRNA_data.index[(scRNA_data.iloc[:,c1]-scRNA_data.iloc[:,c2])>gene_std]]))
        gene_inds2 = np.array(list(genes_table.loc[scRNA_data.index[(scRNA_data.iloc[:,c2]-scRNA_data.iloc[:,c1])>gene_std]]))
        ...
        gene_inds = np.array(list(genes_table.loc[scRNA_data.index[(scRNA_data.iloc[:,c2]-scRNA_data.iloc[:,c1])>gene_std]]))
