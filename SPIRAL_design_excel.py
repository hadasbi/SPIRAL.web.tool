...
def design_excel(sigfile):
    ...
    if ws.max_row>1:
        col_list = list(string.ascii_uppercase)+['A'+letter for letter in list(string.ascii_uppercase)]+['B'+letter for letter in list(string.ascii_uppercase)]
        for col in col_list[1:ws.max_column]:
            if ws[col+'1'].value in ['proc_link', 'func_link', 'comp_link', 'networks', 'UMAPs', 'PCAs',
                ...
                ws.conditional_formatting.add(col+'2:'+col+str(ws.max_row),
                                              ColorScaleRule(start_type='percentile', start_value=10, start_color='00FF00',
